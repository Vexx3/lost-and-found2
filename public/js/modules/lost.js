...
export async function loadLostItems() {
    ...
    try {
        ...
        items
            ...
            const card = h("div", { class: "item card" }, [
                ...
                h("div", { class: "meta" }, [
                    ...
                    h("div", { style: "margin-top:4px;color:#6b7280;font-size:12px" }, `Category: ${categoryLabel(item.category || inferCategoryFromName(item.itemName))}`),
                    secondaryPhoto
        ...
        if (items.length === 0) {
            list.appendChild(h("div", { style: "color:#6b7280;text-align:center;padding:24px" }, "No lost items match your search. Keep checking back."));
        }
    }
    catch (err) {
        console.error(err);
...
if (sb)
    sb.addEventListener("input", loadLostItems);
if (cf)
    cf.addEventListener("change", loadLostItems);
...
async function openClaimForm(itemId) {
    try {
        ...
        if (claimItemNameEl)
            claimItemNameEl.textContent = item.itemName;
...
if (claimCancelSecondaryBtn)
    claimCancelSecondaryBtn.addEventListener("click", closeClaimModal);
if (claimForm) {
