...
let regMediaStreamGlobal: MediaStream | null = null;
let regVideoWrapGlobal: HTMLElement | null = null;
...
export function stopRegCamera() {
  ...
  if (regVideoWrapGlobal) regVideoWrapGlobal.innerHTML = "";
}
...
export function bindRegister() {
  ...
  const previewImg = document.getElementById("regPreview") as HTMLImageElement;
  ...
  let regCapturedDataUrl: string | null = null;
  ...
  async function startRegCamera() {
    try {
      if (regMediaStreamGlobal) return;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const strand = (document.getElementById("strand") as HTMLInputElement).value.trim();
    const email = (document.getElementById("email") as HTMLInputElement).value.trim();
    const contact = (document.getElementById("contact") as HTMLInputElement).value.trim();
    const studentId = (document.getElementById("studentId") as HTMLInputElement).value.trim();
    ...
    try {
      ...
      const existing = userItems.some(
        (x: any) => (x.itemName || "").trim().toLowerCase() === itemName.toLowerCase()
      );
      ...
      const sidInput = document.getElementById("myStudentId") as HTMLInputElement;
