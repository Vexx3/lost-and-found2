...
let scannedItemId: string | null = null;
let scanning = false;
...
export function bindScan() {
  ...
  const foundForm = document.getElementById("foundForm") as HTMLFormElement;
  const imgFile = document.getElementById("img-file") as HTMLInputElement;
  const foundPhotoInput = document.getElementById("foundPhoto") as HTMLInputElement;
  const foundPreview = document.getElementById("foundPreview") as HTMLImageElement;
  ...
  if (resultCard) resultCard.innerHTML = "<em style='color:#6b7280'>Waiting for QR scan&hellip;</em>";
  ...
  async function startCameraScan() {
    try {
      ...
      startJsQRLoop(video, canvas, ctx!, resultCard as HTMLElement);
    } catch (e) {
  ...
  async function decodeFromImage(file: File) {
  ...
  if (foundForm) {
    foundForm.addEventListener("submit", async (e) => {
      ...
      const finderName = (document.getElementById("finderName") as HTMLInputElement).value.trim();
      const location = (document.getElementById("foundLocation") as HTMLInputElement).value.trim();
      const photoFile = (document.getElementById("foundPhoto") as HTMLInputElement).files?.[0];
