...
(function () {
  ...
  function init() {
    const db = { items: [], found_reports: [], claims: [], seq: { found_reports: 0, claims: 0 } };
    save(db);
    return db;
  }
  ...
  function listLostItems() {
    const db = load();
    return db.items.filter((i) => i.status === 'lost').sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1));
  }

  function addFoundReport({ itemId, finderName, location, photoDataUrl }) {
    const db = load();
    db.seq.found_reports = (db.seq.found_reports || 0) + 1;
    const fr = { id: db.seq.found_reports, itemId, finderName, location, photoPath: photoDataUrl || null, status: 'pending', createdAt: nowIso() };
  ...
  function listPendingReportsWithItem() {
    ...
    return db.found_reports
      ...
      .map((r) => {
        ...
        return { ...r, itemName: it?.itemName || 'Unknown', ownerName: it?.ownerName || '', itemPhoto: it?.photoPath || null };
      });
  }
  ...
  function listClaimsWithItem() {
    ...
    return db.claims
      ...
      .map((c) => {
        ...
        return { ...c, itemName: it?.itemName || '', ownerName: it?.ownerName || '', studentId: it?.studentId || '' };
      });
  }
  ...
  function analytics() {
    ...
    const pendingReports = db.found_reports.filter((r) => r.status === 'pending').length;
  ...
  function importMerge(data) {
    ...
    // fix sequences
    db.seq.found_reports = Math.max(db.seq.found_reports || 0, ...(db.found_reports.map((x) => x.id).concat(0)));
    db.seq.claims = Math.max(db.seq.claims || 0, ...(db.claims.map((x) => x.id).concat(0)));
    save(db);
    return true;
  }
  ...
  function qrUrlFor(itemId) {
    ...
    return `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(itemId)}`;
  }
