...
function h(tag, attrs = {}, children = []) {
  ...
  for (const [k, v] of Object.entries(attrs)) {
    if (k === 'class') {
      ...
      // Support event listeners like onclick, oninput, etc.
  ...
  try {
    ...
    reports.forEach((r) => {
      const card = h('div', { class: 'card', style: 'margin-bottom:8px' }, [
        ...
        h('div', { style: 'display:flex; gap:12px; margin-top:8px; align-items:flex-start' }, [
          r.photoPath ? h('div', {}, [
            h('div', { style: 'font-size:12px;color:#6b7280' }, 'Finder Photo'),
            h('img', { src: assetUrl(r.photoPath), style: 'max-width:200px;border-radius:6px;margin-top:4px' })
          ]) : null,
          r.itemPhoto ? h('div', {}, [
            h('div', { style: 'font-size:12px;color:#6b7280' }, 'Owner Photo'),
            h('img', { src: assetUrl(r.itemPhoto), style: 'max-width:200px;border-radius:6px;margin-top:4px' })
          ]) : null,
        ]),
        h('div', { style: 'margin-top:10px' }, [
          h('button', { type: 'button', class: 'btn primary', onclick: async (ev) => {
  ...
  try {
    ...
    claims.forEach((c) => {
      container.appendChild(h('div', { class: 'card', style: 'margin-bottom:8px' }, [
        ...
        h('div', { style: 'font-size:12px;color:#6b7280' }, `On: ${new Date(c.createdAt).toLocaleString()}`),
      ]));
    });
  } catch (e) {
    console.error(e);
