...
const ifoundDB = {
  db: { items: [], found_reports: [], claims: [] } as any,
  async init() {
  ...
  listPendingReportsWithItem() {
    return this.db.found_reports.filter((r: any) => r.status === "pending").map((r: any) => {
      const item = this.db.items.find((i: any) => i.id === r.itemId) || {};
  ...
  listClaimsWithItem() {
    return this.db.claims.map((c: any) => {
      const item = this.db.items.find((i: any) => i.id === c.itemId) || {};
      const report = this.db.found_reports.find((r: any) => r.itemId === c.itemId && r.status === "verified");
      return { 
        ...c, 
  ...
  analytics() {
    ...
    const lost = this.db.items.filter((i: any) => i.status === "lost").length;
    const claimed = this.db.items.filter((i: any) => i.status === "claimed").length;
    const archived = this.db.items.filter((i: any) => i.status === "archived").length;
    const pendingReports = this.db.found_reports.filter((r: any) => r.status === "pending").length;
    return { total, lost, claimed, archived, pendingReports, recoveryRate: total ? Math.round((claimed / total) * 100) : 0 };
  },
  listArchivedItems() {
    return this.db.items.filter((i: any) => i.status === "archived");
  },
  async archiveExpiredItems(days: number) {
    const now = Date.now();
    const ms = days * 24 * 60 * 60 * 1000;
    let count = 0;
    this.db.items.forEach((item: any) => {
...
function loadRetentionPolicy() {
  ...
  const select = document.getElementById("retentionDays") as HTMLSelectElement;
